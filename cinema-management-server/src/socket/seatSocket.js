...
async function clearAllHolds() {
  try {
    ...
    if (keys.length > 0) {
      ...
      console.log(`🧹 Cleared ${keys.length} held seats from previous session`);
    } else {
      console.log("🧹 No held seats to clear");
    }
  } catch (error) {
...
// ✅ THÊM async VÀO ĐÂY
async function setupSeatSocket(io) {
  // ✅ GỌI CLEAR TRƯỚC KHI SETUP
  await clearAllHolds();
  ...
  seatNamespace.on("connection", (socket) => {
    ...
    socket.on("disconnect", async () => {
      ...
      if (socket.showtimeId) {
        await releaseAllSeatsForSocket(
          socket.showtimeId,
          socket.id,
          seatNamespace
        );
